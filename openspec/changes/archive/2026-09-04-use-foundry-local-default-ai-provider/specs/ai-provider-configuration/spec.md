...
- **THEN** the system rejects the request and reports that the AI model is not ready yet, without crashing the application

#### Scenario: Analysis attempted after model preparation failed
- **GIVEN** the Foundry Local provider is selected and preparing the configured model failed
- **WHEN** a user submits a proposal for analysis
